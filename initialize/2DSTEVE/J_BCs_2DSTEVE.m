...
%  Exit(:,:,it)=20e-3*ones(llon,llat);   %V/m
...
pk=1e-6;
lambda=1.5;    %width of structure in degrees
x2ctr=1/2*(xg.x2(lx2)+xg.x2(1));
for it=1:lt
    Vminx1it(:,:,it)=zeros(llon,llat);
    %    Vmaxx1it(:,:,it)=pk.*erf((MLON-mlonmean)/mlonsig);%.*erf((MLAT-mlatmean)/mlatsig);
    Vmaxx1it(:,:,it)=-1*pk.*sin(2*pi/lambda*(MLON-mlonmean));
    for imlon=1:llon
        for imlat=1:llat
            if (abs(MLON(imlon,imlat)-mlonmean)>lambda/2)
                Vmaxx1it(imlon,imlat,it)=0;
            end %if
        end %for
    end %for
