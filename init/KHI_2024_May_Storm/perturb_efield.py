...
def init_profile(xg: dict[str, T.Any], dat: xarray.Dataset) -> np.ndarray:
    from gemini3d.plasma import chapmana    
    ...
    composition = np.zeros( (lsp,lx1) )
    neref=dat["ns"][-1,:,ix2,ix3]
    for i in range(lsp):
    ...
    for i in range(lsp):
        nsprof[i,:]=composition[i,:] * neprof

    nsscale = np.zeros_like(dat["ns"])
    for i in range(lsp):
        for j in range(xg["lx"][1]):
            for k in range(xg["lx"][2]):
                nsscale[i,:,j,k]=nsprof[i,:]
...
def perturb_density(
    ...
    for i in range(lsp):
        for ix2 in range(xg["lx"][1]):
            ...
            nsperturb[i, :, ix2, :] = (
                nsscale[i, :, ix2, :]
                * 5 * (1/2 + 1/2*np.tanh((x2[ix2]) / params["ell"]) )
            )
            # background density
