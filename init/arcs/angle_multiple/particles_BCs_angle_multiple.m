...
displace=10*mlatsig;
mlatctr=precip.mlat_mean+displace*tanh((precip.MLON-precip.mlon_mean)/(mlonsig));
...
for it=1:lt
  ...
  shapefn=shapefn+exp(-(precip.MLON-precip.mlon_mean).^2/2/mlonsig^2).*exp(-(precip.MLAT-mlatctr+3*mlatsig).^2/2/mlatsig^2);
  Qittmp=Qpk.*shapefn;
