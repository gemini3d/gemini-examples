...
mlatctr=E.mlatmean+displace*tanh((E.MLON-E.mlonmean)/(E.mlonsig));
for it=1:Nt
    E.flagdirich(it)=0;
    E.Vminx1it(:,:,it)=zeros(E.llon,E.llat);
    if (it>2)
      E.Vmaxx1it(:,:,it)=-Jpk.*exp(-(E.MLON-E.mlonmean).^2/2/E.mlonsig^2).*exp(-(E.MLAT-mlatctr-3*E.mlatsig).^2/2/E.mlatsig^2);
      E.Vmaxx1it(:,:,it)=E.Vmaxx1it(:,:,it)+Jpk.*exp(-(E.MLON-E.mlonmean).^2/2/E.mlonsig^2).*exp(-(E.MLAT-mlatctr-1.5*E.mlatsig).^2/2/E.mlatsig^2);
      E.Vmaxx1it(:,:,it)=E.Vmaxx1it(:,:,it)-Jpk.*exp(-(E.MLON-E.mlonmean).^2/2/E.mlonsig^2).*exp(-(E.MLAT-mlatctr+1.5*E.mlatsig).^2/2/E.mlatsig^2);
      E.Vmaxx1it(:,:,it)=E.Vmaxx1it(:,:,it)+Jpk.*exp(-(E.MLON-E.mlonmean).^2/2/E.mlonsig^2).*exp(-(E.MLAT-mlatctr+3*E.mlatsig).^2/2/E.mlatsig^2);
    else
