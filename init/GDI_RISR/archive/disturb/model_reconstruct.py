...
def interp_amisr(amisr_file, iso_time, coords):
    ...
    data_check = np.array([chi2 > 0.1, chi2 < 10.0, np.isin(fitcode, [1, 2, 3, 4])])
    ...
    # points = beamcode[:,1:3]
    r = np.cos(beamcode[:, 2] * np.pi / 180.0)
    t = beamcode[:, 1] * np.pi / 180.0
    ...
    for clust_index in tri.simplices:
        ...
        try:
            ...
            coeffs, _ = curve_fit(
                chapman_piecewise,
                a[np.isfinite(d) & ~((a > 400.0 * 1000) & (dd < 5.0e10) & (d < 5.0e10))],
                d[np.isfinite(d) & ~((a > 400.0 * 1000) & (dd < 5.0e10) & (d < 5.0e10))],
                sigma=dd[np.isfinite(d) & ~((a > 400.0 * 1000) & (dd < 5.0e10) & (d < 5.0e10))],
                p0=[4.0e11, 100.0 * 1000.0, 50.0 * 1000.0, 300.0 * 1000.0],
                bounds=[[0.0, 0.0, 0.0, 0.0], [np.inf, 5.0e6, np.inf, np.inf]],
                absolute_sigma=True,
            )
            # print(coeffs)
        except RuntimeError:
    ...
    r = np.cos(clust_el * np.pi / 180.0)
    t = clust_az * np.pi / 180.0
    x = 300.0 * 1000.0 / np.sin(clust_el * np.pi / 180.0) * r * np.sin(t)
    y = 300.0 * 1000.0 / np.sin(clust_el * np.pi / 180.0) * r * np.cos(t)
    ...
    yc = np.mean(y)
    r = 400.0 * 1000.0
    th = np.linspace(0.0, 2 * np.pi, 50, endpoint=False)
    xe = xc + r * np.cos(th)
    ye = yc + r * np.sin(th)
    de = np.full(xe.shape, 1.6e11)
    ...
    dgrid[dist > 400.0 * 1000.0] = 1.6e11
...
def main():
    ...
    coords = [
        np.linspace(-300.0, 500.0, 50) * 1000.0,
        np.linspace(-200.0, 600.0, 50) * 1000.0,
        np.linspace(100.0, 500.0, 30) * 1000.0,
    ]
