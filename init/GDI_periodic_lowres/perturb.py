...
def perturb(cfg: T.Dict[str, T.Any], xg: T.Dict[str, T.Any]):
    ...
    dat = gemini3d.read.state(cfg["indat_file"])
    lsp = dat["ns"].shape[0]

    # %% Choose a single profile from the center of the eq domain
    ix2 = xg["lx"][1] // 2
    ix3 = xg["lx"][2] // 2
    nsscale = np.zeros(dat["ns"].shape)
    for i in range(lsp):
        nprof = dat["ns"][i, :, ix2, ix3]
        nsscale[i, :, :, :] = nprof
    ...
    nsperturb = np.zeros(dat["ns"].shape)
    for i in range(lsp - 1):
        for j in range(xg["lx"][1]):
            amplitude = numpy.random.standard_normal([xg["lx"][0], 1, xg["lx"][2]])
            ...
            nsperturb[i, :, j, :] = nsscale[
                i, :, j, :
            ] + nepatchfact * nsscale[  # original data
                i, :, j, :
            ] * (
                1 / 2 * np.tanh((x2[j] - x21) / ell)
                - 1 / 2 * np.tanh((x2[j] - x22) / ell)
            )
            # patch, note offset in the x2 index!!!!

            if (j > 10) & (j < xg["lx"][1] - 10):
                # do not apply noise near the edge (corrupts boundary conditions)
                nsperturb[i, :, j, :] = (
                    nsperturb[i, :, j, :] + amplitude * nsscale[i, :, j, :]
                )

    nsperturb = np.maximum(nsperturb, 1e4)
    # enforce a density floor (particularly need to pull out negative densities which can occur when noise is applied)
    nsperturb[lsp, :, :, :] = nsperturb[:-1, :, :, :].sum(axis=0)
