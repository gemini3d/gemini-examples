...
direc="/Users/zettergm/simulations/ssd_ext/CI/nightQ/Disturb_nonightQ/"
...
for it in range(0, len(cfg["time"])):
    ...
    plt.clf()
    #plt.subplot(1, 3, 1)
    plt.pcolormesh(mlati, malti, np.log10(parmmi[:, llon // 2, :]))
    plt.ylim(0, 1000e3)
    plt.xlabel("mlat")
    plt.ylabel("alt")
    plt.title("$n_e$")
    ...
    filename=plotdir + "/" + (parmlbl + str(cfg["time"][it]) + "s.png")
