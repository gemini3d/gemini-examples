...
from potential_parameterize_ellipse import density_ellipse


def perturb_constL(cfg: T.Dict[str, T.Any], xg: T.Dict[str, T.Any]):
    """
    ...
    x3ctr = 0e3
    nref=4
    nend=1     # by using a background of "1" we can create a shape function for multiplying the input
    a=2*ellx2
    b=6*a
    [X2,X3]=np.meshgrid(x2,x3,indexing='ij')
    nshape,_ = density_ellipse(X2,X3,x2ctr,x3ctr,a,b,ellx2,nref,nend)
    ...
    for i in range(lsp - 1):
        for j in range(xg["lx"][1]):
            for k in range(xg["lx"][2]):
                ...
                # # Circular patch
                # nsperturb[i, :, j, k] = nsscale[i, :, j, k] + nepatchfact * nsscale[i, :, j, k] * (
                #     np.exp(-(1 - (np.sqrt( ((x2[j]-x2ctr)**2) / 1 + (x3[k]**2) / 6 )) )**8/2/ellx2**8)
                # )
                # # patch, note offset in the x2 index!!!!
                
                nsperturb[i,:,j,k]= nsscale[i,:,j,k]*nshape[j,k]
